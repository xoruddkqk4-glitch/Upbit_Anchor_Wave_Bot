...
* `ref_manager.py`: 수동 기준봉 사전 검증, 지표 자동 산출 및 bot_state.json 동시성 안전 관리 모듈
* `telegram_commander.py`: 텔레그램 번호/버튼 선택형 수동 기준봉 등록 및 현황 조회 대화형 데몬
...
- **수정 내용** (돌파 매매 손절선 고도화: 기준봉 고가 2% 완충 버퍼 및 진입가 -5% 비상 상한선 하이브리드 전략 적용, 최대 매수 한도 50만 원 문서 동기화, GEMINI.md 생성, 파일: `Upbit_Anchor_Wave_Bot.py`, `GEMINI.md`, `README.md`):
  1. **돌파 매매 완충 버퍼 손절선 파라미터 신설 (`Upbit_Anchor_Wave_Bot.py`)**:
     ...
     - `BREAKOUT_MAX_LOSS_PCT = 0.05`: 급등 추격 진입 시 최대 손실을 진입가 대비 -5.0%로 제한하는 2차 안전망 유지
  2. **돌파 손절선 동적 산출 엔진 고도화 (`calc_breakout_stop`)**:
     - 기존 확정봉 저가 방식에서 `ref_high` 기반 완충 버퍼(`buffer_stop = ref_high * 0.98`)와 최대 손실 캡(`loss_cap = entry_price * 0.95`)을 비교하여 더 타이트한 상단 가격 선택 (`max(buffer_stop, loss_cap)`)
     - 기존 손절선보다는 낮아지지 않도록 상향 이동 원칙 보장
  3. **돌파 진입 및 추가 매수 호출부 연동**:
     - 신규 돌파 매수(`BUY (BREAKOUT ALL-IN)`)의 변동성 사이징 예상 손절가 산출 및 체결 후 손절선에 `ref_high` 연동
     - 눌림목 보유 중 잔액 돌파 매수(`BUY (BREAKOUT FULL SCALE-IN)`)의 갱신 평단가 및 `ref_high` 기준 손절선 재조정 연동
  4. **에이전트 환경 설정 동기화**:
     - 워크스페이스 실행 규칙 `AGENTS.md`와 동일한 `GEMINI.md` 생성 및 `.agents` 규칙 체계 완전 연동
  5. **README 개요 및 함수 독스트링 최대 매수 금액 정정**:
     - 과거 100만 원으로 표기되어 있던 개요 문구 및 `calc_position_size` 독스트링을 현재 설정값인 50만 원(`MAX_BUY_AMOUNT_KRW = 500,000`, 1회 손절 손실 5만 원)으로 일치화 정정
- **검증 결과**:
  - `python -m py_compile Upbit_Anchor_Wave_Bot.py` 정적 구문 검사 통과 (Exit Code 0)
  - **단위 시나리오 검증 통과 (`assert` 완료)**:
    - [테스트 1] 일반 돌파 진입 시: 고가 1,000원, 진입가 1,010원 -> 손절가 980원 (`기준봉 고가 -2.0% 버퍼 적용`, 실질 손실 -2.97%) 정상 산출
    - [테스트 2] 급등 추격 진입 시: 고가 1,000원, 진입가 1,080원 -> 손절가 1,026원 (`진입가 -5% 상한 적용`) 정상 방어
    - [테스트 3] 기존 손절선이 높은 경우: 기존 990원 유지 정상 산출
  - `.env`, `service_account.json` 비밀 파일 Git 미추적 상태 유지 확인 (`AUTO_TRADE_EXECUTE` 미변경, 실주문 API 미호출)

## 📝 [2026-09-23 08:52] 업데이트 이력 (Commit ID: d7699b8)
- **수정 내용** (오타 방지 번호 선택형 수동 기준봉 지정 시스템 구축 및 텔레그램 대화형 커맨더 데몬 탑재, 파일: `ref_manager.py` 신규, `telegram_commander.py` 신규, `test_manual_ref.py` 신규, `README.md`):
  1. **수동 기준봉 사전 검증 및 지표 자동 산출 코어 모듈 신설 (`ref_manager.py`)**:
     - 기준봉 자동 판정(거래량 200%, 상승률 10%, 20일 전고점 돌파)에 미세하게 미달하여 탈락한 주도주를 살려내기 위해 수동 등록 기능 구축
     - **오타 방지 미등록 코인 번호 매핑 (`get_available_tickers`)**: 감시 대상 20개 종목(`TARGET_TICKERS`) 중 활성 기준봉이 없고 미보유 상태인 코인들만 필터링하여 1번부터 순번을 자동 부여
     - **사전 엄격 검증 (`validate_and_register_manual_ref`)**:
       - 마감 확정 일봉 검증 (당일 진행 중 캔들 및 미래 일자 차단)
       - 유효기간 20일(`REF_EXPIRY_DAYS`) 초과 여부 확인
       - **사후 손절선 기이탈 검증 (`broken_in_history`)**: 기준봉 발생 이후 전일까지의 저가가 기준봉 손절선(저가)을 깬 적이 있는지 검사하여 구조가 파괴된 캔들 자동 차단
       - 실시간 현재가가 이미 손절가 미만인 종목 사전 차단
     - **지표 자동 계산**: 고가, 갭보정 손절가, 중심가(50%), 20일 스윙 저점, 1차 파동 높이를 봇 알고리즘과 100% 동일하게 산출
     - **동시성 안전 저장**: `state_lock.py`의 `StateFileLock`을 활용해 5분 주기 봇 및 09:07 스캐너 실행 중에도 안전하게 `bot_state.json` 갱신
  2. **대화형 텔레그램 수신 데몬 구축 (`telegram_commander.py`)**:
     - **터치식 인라인 버튼 + 텍스트 번호 입력 동시 지원**: `/setref` (또는 `/기준봉`) 호출 시 3열 인라인 버튼 그리드 및 번호 목록 전송 ➔ 원터치 또는 번호 입력으로 오타 없는 코인 선택
     - **날짜 입력 대화 세션 관리**: 코인 선택 후 `YYYY-MM-DD` 날짜 입력 시 즉시 업비트 차트 조회 및 유효성 검증 결과를 사용자에게 상세 회신
     - **단축 명령어 지원**: `/setref 2 2026-03-15` 또는 `/setref KRW-BTC 2026-03-15` 로 즉시 등록 지원
     - **자가 치유(Self-healing) 웹훅 자동 해제 탑재**: 시작 시 및 HTTP 409 Conflict 발생 시 기존 잔존 웹훅을 자동 감지하고 `deleteWebhook`을 안전하게 호출하여 롱폴링 충돌 원천 차단
     - **현황 및 취소 명령어**: `/status` (포지션/기준봉 현황), `/cancelref` (기준봉 수동 해제), `/cancel` (세션 취소)
  3. **단위 테스트 스위트 구축 및 전원 검증 완료 (`test_manual_ref.py`)**:
     - 미등록 코인 필터링, 유효기간 초과 거절, 사후 손절선 이탈 거절, 정상 등록 키 무결성, 기준봉 수동 취소, 텔레그램 인라인/번호 파싱 등 8종 단위 테스트 전원 통과
- **검증 결과**:
  - `python -m py_compile telegram_commander.py ref_manager.py test_manual_ref.py` 통과 (Exit Code 0)
  - `python -m unittest test_manual_ref.py` 8개 단위 테스트 100% PASS
